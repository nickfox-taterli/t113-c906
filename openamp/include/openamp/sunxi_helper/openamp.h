...
struct rpmsg_driver {
	...
	int (*probe)(struct rpmsg_device *rdev, struct rpmsg_endpoint *ept,
					uint32_t src, uint32_t dest);
	void (*remove)(struct rpmsg_endpoint *ept);
	int (*callback)(struct rpmsg_endpoint *ept, void *data, size_t len,
					uint32_t src, void *priv);
	void *priv;			/* used by user */
};
