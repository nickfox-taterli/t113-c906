...
static inline int __metal_mutex_try_acquire(metal_mutex_t *mutex)
{
	...
	if (xSemaphoreTake(mutex->v, 0) == pdTRUE) {
...
{
	if (xSemaphoreTake(mutex->v, portMAX_DELAY) == pdTRUE) {
		mutex->islock = METAL_MUTEX_LOCKED;
	}
}
