...
//	openamp_dbg("notifyid=%d try to match vdev=%d\r\n", notifyid,
//					vdev->notifyid);
//	if (vdev->notifyid == notifyid) {
//		openamp_dbg("vdev notifyid match (do nothing)\r\n");
//		return 0;
//	}
	num_vrings = vdev->vrings_num;
	for (i = 0; i < num_vrings; i++) {
...
void rproc_virtio_early_wait_remote_ready(struct fw_rsc_vdev *vdev_rsc,
				struct metal_io_region *io, unsigned int role)
{
	uint8_t status;
