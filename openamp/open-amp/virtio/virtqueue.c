...
//#define VRING_FLUSH(x)		openamp_dbg("cache fulsh:%p:0x%x", &x, sizeof(x));metal_cache_flush(&x, sizeof(x))
//#define VRING_INVALIDATE(x)	openamp_dbg("cache inval:%p:0x%x", &x, sizeof(x));metal_cache_invalidate(&x, sizeof(x))
...
//	openamp_dbg("vq->vq_ring.used->idx(%p)=%d\r\n", (uint32_t *)&(vq->vq_ring.used->idx),
//					vq->vq_ring.used->idx);

//	uint32_t *r = (uint32_t *)&(vq->vq_ring.used->idx);
//	openamp_dbg("rad(%p)=%d\r\n", r,*r);
